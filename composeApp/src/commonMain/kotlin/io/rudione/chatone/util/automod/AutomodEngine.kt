...
object AutomodEngine {
    ...
    private fun matchRule(rule: AutomodRule, text: String): String? {
        val scanText = if (rule.ignoreLinks) stripLinks(text) else text
        val haystack = if (rule.caseSensitive) scanText else scanText.lowercase()
        for (pattern in rule.allPatterns) {
            val needle = if (rule.caseSensitive) pattern else pattern.lowercase()
            val hit = if (rule.isRegex) {
                regexFor(pattern, rule.caseSensitive)?.containsMatchIn(scanText) == true
    ...
    private fun stripLinks(text: String): String {
        var result = text
        for (match in ChatRuleEngine.LINK_RE.findAll(text)) {
            result = result.replaceRange(match.range, " ".repeat(match.value.length))
        }
        return result
    }
